...
由于缺乏坐标数据，无法实时判断“能否拆包”（距离/掩体）。因此，引入**基于结果的回溯模拟机制**。
在解析每一回合时，我们预先知道该回合的最终结果（拆包成功 或 拆包失败），并据此应用不同的胜率计算模型。

### 5.1 场景 A：拆包成功 (Defuse Success)
*   **模拟逻辑**:
    *   **全歼锁定**: 当 $N_T \to 0$ 时，强制锁定 $P(T) = 0.0$ (即 CT 胜率 100%)。
    *   **时间修正**: 应用与场景 B 类似的数学模型，但**始终保持缓慢衰减**，不会随时间流逝而急剧降低 CT 胜率（因为已知最终拆包成功，说明时间是足够的）。
*   **WPA 分配**:
    *   **常规情况 (5v0 拆包)**:
        *   击杀最后一个 T 时，CT 胜率已跳变至 100%。
        *   拆包动作发生时，$\Delta P = 1.0 - 1.0 = 0$。拆包不加分。
        *   分数全部归于击杀者。
    *   **例外情况 (非 100% 胜率拆包 / Ninja Defuse)**:
        *   若拆包**开始**时，$P(CT) < 100\%$ (例如 T 存活)，则触发例外分配。
        *   **拆包者奖励**: 获得从拆包开始至拆包结束（成功）期间所有胜率增量 ($\Delta P_{total}$) 的 **35%**。
        *   **击杀者/存活者**: 获得剩余的 65%。
        *   这确保了在极限情况下（如顶着火力拆包），拆包者能获得应有的战术贡献分。

### 5.2 场景 B：拆包失败 (Bomb Exploded)
*   **模拟逻辑**:
    *   引入**回防乘数 $x$**，作用于 CT 的基础胜率。
    *   $$ P(CT)_{final} = P(CT)_{matrix} \times x(t) $$
    *   **乘数 $x(t)$ 曲线**:
        *   **起点**: 下包瞬间 (倒计时 40s)，$x = 1.0$。
        *   **终点**: 炸弹不可拆除点 (倒计时 ~1s)，$x = 0.0$。
        *   **衰减速率**:
            *   **前 24s**: 缓慢衰减 (CT 仍有很大机会回防)。
            *   **后 16s**: 快速衰减 (时间告急，CT 胜率雪崩)。
*   **WPA 分配**:
    *   **时间衰减**: 由所有存活的 CT 玩家共同承担负分（因为他们未能及时回防）。
    *   **击杀事件**: 仍然正常结算瞬间的 $\Delta P$。但由于 $x(t)$ 的存在，随着时间推移，击杀带来的收益会越来越小（因为 $P(CT)$ 上限在不断降低）。
