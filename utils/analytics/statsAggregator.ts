...
export const aggregatePlayerStats = (
  ...
  const stats: AggregatedStats = {
    ...
    sniperKills: 0,
    sniperMultiKillRounds: 0,
    sniperOpeningKills: 0,
    roundsWithSniperKills: 0,
  };
  ...
  history.forEach(({ match, stats: summaryStats }) => {
    ...
    const matchPlayer = [...match.players, ...match.enemyPlayers].find(
      (p) =>
    ...
    match.rounds.forEach((round) => {
      ...
      if (pRound.utility && pRound.utility.flashesThrown > 0) {
        roundFlashAssists = round.timeline.filter(
          (e) =>
            e.type === "flash_assist" &&
            e.subject &&
      ...
      if (pRound.utility) {
        stats.utilityDamage +=
          (pRound.utility.heDamage || 0) + (pRound.utility.molotovDamage || 0);
      ...
      const recentDamageTaken = new Map<
        string,
        { victim: string; time: number }
      >(); // AttackerID -> LastVictimInfo

      let roundSniperKills = 0;
      let hasSniperOpening = false;

      if (round.timeline) {
        const playerKills = round.timeline.filter(
          (e) =>
            e.type === "kill" &&
            e.subject &&
            (e.subject.steamid === targetId || e.subject.name === targetId),
        ...
        const isRoundFirstKill =
          firstKillOfRound && firstKillOfRound === playerKills[0];

        round.timeline.forEach((e) => {
          const subjectId = e.subject
            ? e.subject.steamid || e.subject.name
            : "";
          ...
          if (e.type === "damage" && subjectId && targetIdStr) {
            ...
            if (e.subject && e.subject.side !== pRound.side) {
              recentDamageTaken.set(subjectId, {
                victim: targetIdStr,
                time: e.seconds,
              });
            }
          ...
          if (e.type === "kill" && subjectId && targetIdStr) {
            ...
            if (killerId === targetId) {
              ...
              if (recentDamageTaken.has(victimId)) {
                ...
                if (
                  hurtPlayerStats &&
                  hurtPlayerStats.side === pRound.side &&
                  info.victim !== targetId
                ) {
      ...
      if (!pRound.survived) {
        ...
        const deathEvent = round.timeline.find(
          (e) =>
            e.type === "kill" &&
            e.target &&
      ...
      const teammates = Object.values(round.playerStats).filter(
        (p) => p.side === pRound.side,
      );
      ...
      if (pRound.survived) {
        ...
        if (deadTeammates === totalTeammates) {
          const teamIds = Object.keys(round.playerStats).filter(
            (id) => round.playerStats[id].side === pRound.side,
          );
          let myDeathTime = 0;
          ...
          round.timeline.forEach((e) => {
            if (e.type === "kill" && e.target) {
              ...
              if (teamIds.includes(tid) && tid !== targetId) {
                maxTeammateDeathTime = Math.max(
                  maxTeammateDeathTime,
                  e.seconds,
                );
              }
            }
