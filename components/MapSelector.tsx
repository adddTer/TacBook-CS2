...
export const MapSelector: React.FC<MapSelectorProps> = ({ currentMap, onChange }) => {
  ...
  return (
    <div className="w-full">
      <div 
        ref={scrollRef}
        className="flex overflow-x-auto no-scrollbar py-2 px-4 gap-2 snap-x"
      >
        {MAPS.map((map) => {
          ...
          return (
            <button
              ...
              className={`
                snap-center shrink-0 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-200 border
                ${isActive 
