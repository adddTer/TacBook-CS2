...
import('mermaid').then(({ default: mermaid }) => {
    ...
    const chart3 = chart.trim().split(';').map(s => s.trim()).filter(Boolean).join('\n');
    mermaid.render('test-3', chart3).then(res => {
        console.log("Success splitted =>", res.svg.substring(0, 100));
    }).catch(e => {
        console.error("FAIL splitted =>", e.message || e);
    });
